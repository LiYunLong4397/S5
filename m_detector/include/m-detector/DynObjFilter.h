...
#include "lidar_perception/min_box/min_box.h"
#include "lidar_perception/hm_tracker/hm_tracker.h"
...
#define MAX_N 100000
#define MAX_2D_N       (564393) //1317755(1317755) //(50086) 39489912 // MAX_1D * MAX_1D_HALF 深度图总格数
#define MAX_1D         (1257) //(2095) //(317)  // 2*pi/ hor_resolution 深度图总列数（长）
#define MAX_1D_HALF    (449) //3142() //158 31416 pi / ver_resolution  深度图总行数（宽）
...
struct point_soph
{
    ...
    {
        //vec为球面坐标系下的坐标
        vec(2)    = float(point.norm());//求r，范数，(x*x+y*y+z*z)^1/2
        //返回point(1)/point(0)即y/x的反正切值 arctan(y/x)，xoy平面，连线与x轴夹角 -pi~pi
        vec(0)    = atan2f(float(point(1)), float(point(0)));
        //arctan(z/(x*x+y*y)^1/2)，点与原点连线和z轴夹角 -pi/2~pi/2
        vec(1)    = atan2f(float(point(2)), sqrt(pow(float(point(0)), 2) + pow(float(point(1)), 2)));
        //以下为深度图中的坐标，向下取整
        hor_ind   = floor((vec(0) + PI_MATH) / hor_resolution_max);//列数
        ver_ind   = floor((vec(1) + 0.5 * PI_MATH) / ver_resolution_max);//行数
        position  = hor_ind * MAX_1D_HALF + ver_ind;//在深度图中，该点的位置（索引）
    };
...
public:
    ...
    {   
        depth_map.assign(MAX_2D_N, std::vector<point_soph*>());//赋值 MAX_2D_N个std::vector<point_soph*>
        time = cur_time;
        project_R = rot;
        project_T = transl;
        min_depth_static = new float[MAX_2D_N];
...
public:
    ...
    std::shared_ptr<apollo::perception::SensorObjects> my_objects;
    ...
    PointCloudXYZI::Ptr laserCloudAllObj;
...
//evaluate dyn point
    ...
    int minClusterSize_;
    int maxClusterSize_;
    std::vector<pcl::PointIndices> clusters_dyn;
    ...
    int minnum_clus;
    bool over_num;
    int cluster_num_thredshold1;
    int cluster_num_thredshold2;
    float disp_shredshold;
    bool luzhang;
    ...
    void filter(PointCloudXYZI::Ptr feats_undistort, const M3D & rot_end, const V3D & pos_end, const double & scan_end_time, const ros::Publisher &cluster_vis_high, const ros::Publisher & pub_pcl_cluster_);
    void publish_dyn(const ros::Publisher & pub_pcl_all, const ros::Publisher & pub_point_out, const ros::Publisher & pub_frame_out, const ros::Publisher & pub_steady_points, const double & scan_end_time);
    void set_path(string file_path, string file_path_origin);

    void LidarPerception(const ros::Publisher & pub_pcl_cluster_);
    void publish_markers(const double & scan_end_time, const ros::Publisher & marker_pub_box_, const ros::Publisher & marker_pub_tracinfo_, const ros::Publisher & marker_pub_track_,  const M3D & rot_end, const V3D & pos_end, const ros::Publisher &pcl_boundary);
    void ClearAllMarker(const ros::Publisher & marker_pub_box_, const ros::Publisher & marker_pub_tracinfo_, const ros::Publisher & marker_pub_track_);
    ...
    void pointcluster(pcl::KdTreeFLANN<PointType>::Ptr &kdtreeCornerLast, int &dyn_num, int &std_num, vector<point_soph*> &points, std::vector<int> &neighbor, double &threshold_dyn, pcl::PointIndices &cl, int &dyn_all);
